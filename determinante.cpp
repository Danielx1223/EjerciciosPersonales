...
for ( i=2; i<=n; i++)
{
    for ((j=1);(j<=n);(j++))
     {
       V[i][j]=(V[i][j]*A[1][1])+((V[1][j]*A[i][1]*(-1)));
       F[i][j]=(F[i][j]*A[1][1])+((F[1][j]*A[i][1]*(-1)));
     }
 j=1;
}
cout<<"\tla matriz va\n";
   ...
   for (a=2; a<=n; a++)//aaaaaaaaaaaaaaaaaaaaaaaaa
     {
       for (j=1;j<=n;j++)
        {
            V[a+1][j]=(V[i][j]*A[a+1][a])+((V[a+1][j]*A[i][a]*(-1)));
            F[a+1][j]=(F[i][j]*A[a+1][a])+((F[a+1][j]*A[i][a]*(-1)));
        }
    }
cout<<"\tla matriz va\n";
for (i=1;i<=n; i++)
